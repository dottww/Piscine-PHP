...
class Vector
{
	...
	function __toString()
	{
		if (Self::$verbose == TRUE) {
			return (sprintf(
				"Vector( x:%0.2f, y:%0.2f, z:%0.2f, w:%0.2f )",
				$this->_x,
				$this->_y,
				$this->_z,
				$this->_w
	...
	{
		$dotProduct = (float)($this->_x * $rhs->get_x() + $this->_y * $rhs->get_y() + $this->_z * $rhs->get_z());
		return $dotProduct;
	}
	
	public function cos(Vector $rhs)
	{
		$cos = (float)($this->dotProduct($rhs) / ($this->magnitude() * $rhs->magnitude()));
		return $cos;
	}
